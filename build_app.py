...
.btn-topbar-github{{background:transparent;color:#24292f;border:1px solid var(--border)}}.btn-topbar-github:hover{{background:#24292f;color:#fff;border-color:#24292f}}
.topbar-divider{{width:1px;height:22px;background:var(--border);flex-shrink:0}}
.github-status-bar{{display:flex;align-items:center;gap:8px;padding:10px 14px;border-radius:var(--radius-sm);margin-bottom:16px;font-size:13px;font-weight:500}}
.github-status-bar.connected{{background:#dcfce7;color:#15803d;border:1px solid #bbf7d0}}
.github-status-bar.error{{background:var(--danger-light);color:var(--danger);border:1px solid #fca5a5}}
.github-status-bar.syncing{{background:var(--blue-light);color:var(--blue);border:1px solid #bfdbfe}}
...
/* Snapshot */
.snapshot-save-row{{display:flex;gap:8px;margin-bottom:20px}}
.snapshot-list{{display:flex;flex-direction:column;gap:8px}}
.snapshot-item{{display:flex;align-items:center;gap:12px;padding:12px 14px;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius-sm);transition:border-color var(--tr)}}
.snapshot-item:hover{{border-color:var(--border-hover)}}
.snapshot-info{{flex:1;min-width:0}}
.snapshot-name{{font-weight:600;font-size:14px;color:var(--text);overflow:hidden;text-overflow:ellipsis;white-space:nowrap}}
.snapshot-meta{{font-size:12px;color:var(--text-muted);margin-top:2px}}
.snapshot-badge{{display:inline-flex;align-items:center;gap:3px;padding:2px 7px;border-radius:20px;font-size:11px;font-weight:600;background:var(--surface2);border:1px solid var(--border);color:var(--text-secondary)}}
.snapshot-actions{{display:flex;gap:6px;flex-shrink:0}}
.snapshot-empty{{text-align:center;padding:24px;color:var(--text-muted);font-size:13px;background:var(--surface2);border-radius:var(--radius-sm);border:1px dashed var(--border)}}
...
<div class="main-wrapper">
  <header class="topbar">
    ...
    <div class="topbar-actions">
      <button class="btn-topbar btn-topbar-icon" id="saveStateTopBtn" title="현재 상태 저장" onclick="quickSaveSnapshot()">
        <svg viewBox="0 0 24 24"><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"/></svg>
        <span>저장</span>
      </button>
      <button class="btn-topbar btn-topbar-icon" id="restoreTopBtn" title="JSON 파일 복원" onclick="document.getElementById('restoreInputTop').click()">
      ...
      <button class="btn-topbar btn-topbar-github" id="githubSyncTopBtn" title="GitHub 동기화" onclick="syncToGitHub()">
  ...
  <main class="main-content">
    ...
    <div class="view" id="view-settings">
      ...
      <!-- 현재 상태 저장 -->
      <div class="settings-section">
        <div class="settings-section-title">💾 현재 상태 저장</div>
        <div class="settings-section-desc">지금 이 순간의 데이터를 저장해두고 언제든 복원할 수 있습니다. 최대 10개까지 보관됩니다.</div>
        <div class="snapshot-save-row">
          <input type="text" class="genre-edit-input" id="snapshot-name-input" placeholder="저장 이름 (비워두면 날짜·시간으로 자동 설정)" maxlength="30">
          <button class="btn btn-primary btn-sm" onclick="saveSnapshotFromInput()" style="white-space:nowrap">
            <svg viewBox="0 0 24 24" style="width:13px;height:13px"><path d="M17 3H5c-1.11 0-2 .9-2 2v14c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V7l-4-4zm-5 16c-1.66 0-3-1.34-3-3s1.34-3 3-3 3 1.34 3 3-1.34 3-3 3zm3-10H5V5h10v4z"/></svg>
            저장
          </button>
        </div>
      ...
      <div class="settings-section">
        ...
        <div class="backup-folder-row">
          ...
          <button class="btn btn-secondary btn-sm" onclick="removeFolderSelection()" title="폴더 지정 해제" style="color:var(--danger);border-color:var(--danger)">✕</button>
        </div>
      ...
      <!-- GitHub 동기화 -->
      <div class="settings-section">
        <div class="settings-section-title">🐙 GitHub 동기화</div>
        <div class="settings-section-desc">GitHub 저장소에 백업 파일을 업로드하거나 복원합니다. Personal Access Token을 발급해 설정하세요.</div>

        <div class="github-status-bar" id="github-status-bar" style="display:none">
        ...
        <div class="form-group">
          ...
          <div style="display:flex;gap:8px">
            <input type="password" class="form-control" id="github-token" placeholder="ghp_xxxxxxxxxxxxxxxxxxxx" autocomplete="off">
            <button type="button" class="btn btn-secondary btn-sm" style="flex-shrink:0;white-space:nowrap" onclick="(()=>{{const i=document.getElementById('github-token');i.type=i.type==='password'?'text':'password'}})()">👁 보기</button>
          </div>
        </div>
        <div class="form-row">
          <div class="form-group">
            <label class="form-label">GitHub 사용자명</label>
            <input type="text" class="form-control" id="github-owner" placeholder="username">
          </div>
          <div class="form-group">
            <label class="form-label">저장소명</label>
            <input type="text" class="form-control" id="github-repo" placeholder="bookshelf-backup">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">파일 경로</label>
          <input type="text" class="form-control" id="github-path" placeholder="bookshelf_backup.json">
          <div style="font-size:11px;color:var(--text-muted);margin-top:4px">저장소 내 경로. 예) data/bookshelf_backup.json</div>
        </div>
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:16px">
          <button class="btn btn-primary btn-sm" onclick="saveGithubSettings()">💾 설정 저장</button>
          <button class="btn btn-secondary btn-sm" onclick="testGithubConnection()">🔍 연결 테스트</button>
          <button class="btn btn-secondary btn-sm" id="btn-github-sync" onclick="syncToGitHub()">🐙 GitHub에 업로드</button>
          <button class="btn btn-secondary btn-sm" id="btn-github-restore" onclick="restoreFromGitHub()">⬇️ GitHub에서 복원</button>
        </div>
        <div id="github-detail-error" style="display:none;margin-bottom:12px;padding:10px 14px;background:#fef2f2;border:1px solid #fca5a5;border-radius:var(--radius-sm);font-size:12px;color:#b91c1c;line-height:1.7;font-family:monospace;white-space:pre-wrap;word-break:break-all"></div>
        <div class="info-box">
          <strong>💡 설정 방법 (3단계):</strong><br>
          ① GitHub에서 빈 저장소 생성 (예: <strong>bookshelf-backup</strong>)<br>
          ② <strong>Settings → Developer settings → Personal access tokens → Tokens (classic)</strong><br>
          &nbsp;&nbsp;&nbsp;→ Generate new token → <strong>repo</strong> 권한 체크 → 토큰 복사<br>
          ③ 위 입력란에 토큰·사용자명·저장소명 입력 후 <strong>설정 저장</strong> → <strong>연결 테스트</strong>로 확인<br>
          <span style="margin-top:4px;display:block;color:#1e40af">⚠️ Fine-grained token은 해당 저장소의 <strong>Contents: Read and write</strong> 권한 필요</span>
        </div>

        <div style="border-top:1px solid var(--border);margin:20px 0"></div>
        <div style="font-size:14px;font-weight:700;color:var(--text);margin-bottom:6px">🌐 URL로 복원 (GitHub Pages / 공개 URL)</div>
        <div style="font-size:13px;color:var(--text-secondary);margin-bottom:12px;line-height:1.6">
          토큰 없이 공개 URL(GitHub Pages 등)에서 백업 파일을 바로 복원합니다.<br>
          GitHub Pages가 설정된 경우 아래 버튼으로 URL을 자동 입력할 수 있습니다.
        </div>
        <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">
          <input type="text" class="form-control" id="restore-url-input" placeholder="https://username.github.io/repo/bookshelf_backup.json" style="flex:1;min-width:240px">
          <button class="btn btn-secondary btn-sm" style="white-space:nowrap" onclick="fillGithubPagesUrl()">📋 GitHub Pages URL 자동 입력</button>
        </div>
        <button class="btn btn-primary btn-sm" onclick="restoreFromUrl()">⬇️ URL에서 복원</button>
      </div>
...
function renderSettings() {{
  ...
  updateFolderDisplay(state.directoryHandle?.name||'');
  populateGenreSelect();
  renderSnapshotList();
  populateGithubForm();
  const syncTime=localStorage.getItem('bookshelf_github_synced');
  if(syncTime) {{
    const s=loadGithubSettings();
    if(s.token) setGithubStatus('connected',`✅ 마지막 업로드: ${{syncTime}}`);
  }}
}}
...
/* ===================== Snapshot ===================== */
const SNAPSHOT_KEY = 'bookshelf_snapshots_v1';
const MAX_SNAPSHOTS = 10;

function loadSnapshots() {{
  try {{ return JSON.parse(localStorage.getItem(SNAPSHOT_KEY)||'[]'); }} catch(e) {{ return []; }}
}}
function persistSnapshots(snaps) {{ localStorage.setItem(SNAPSHOT_KEY, JSON.stringify(snaps)); }}
...
function createSnapshot(customName) {{
  ...
  const snap={{
    id: now.getTime(),
    name: customName||(dateStr+' '+timeStr),
    date: dateStr, time: timeStr,
    bookCount: state.books.length,
    completedCount: state.books.filter(b=>b.completed).length,
    readingCount: state.books.filter(b=>b.reading&&!b.completed).length,
    data: {{books: JSON.parse(JSON.stringify(state.books)), genres:[...state.genres]}}
  }};
...
function saveSnapshotFromInput() {{
  const input=document.getElementById('snapshot-name-input');
  const name=(input?.value||'').trim();
  const snap=createSnapshot(name);
  if(input) input.value='';
  showToast(`💾 저장되었습니다: ${{snap.name}}`);
  renderSnapshotList();
}}

function quickSaveSnapshot() {{
  const snap=createSnapshot();
  showToast(`💾 현재 상태 저장 완료 (${{snap.name}})`);
  if(state.currentView==='settings') renderSnapshotList();
}}

function restoreSnapshot(id) {{
  const snap=loadSnapshots().find(s=>s.id===id);
  if(!snap) return;
  if(!confirm(`"${{snap.name}}" 시점으로 복원하시겠습니까?\\n현재 데이터(${{state.books.length}}권)가 교체됩니다.`)) return;
  state.books=snap.data.books;
  state.genres=snap.data.genres||[...DEFAULT_GENRES];
  saveBooks(); saveGenres();
  showToast(`✅ "${{snap.name}}" 시점으로 복원되었습니다.`);
  setTimeout(()=>location.reload(),1500);
}}

function deleteSnapshot(id) {{
  persistSnapshots(loadSnapshots().filter(s=>s.id!==id));
  renderSnapshotList();
...
function renderSnapshotList() {{
  ...
  const remain=MAX_SNAPSHOTS-snaps.length;
  if(!snaps.length) {{
    el.innerHTML='<div class="snapshot-empty">저장된 상태가 없습니다.<br>상단 버튼으로 현재 상태를 저장해보세요.</div>';
    return;
  }}
    ...
    <div class="snapshot-item">
      <div class="snapshot-info">
        ...
        <div class="snapshot-meta">
          ${{esc(s.date)}} ${{esc(s.time||'')}} &nbsp;
          <span class="snapshot-badge">📚 ${{s.bookCount}}권</span> &nbsp;
          <span class="snapshot-badge" style="background:var(--green-light);color:var(--green);border-color:#bbf7d0">✅ ${{s.completedCount}}</span>
          ${{s.readingCount?`&nbsp;<span class="snapshot-badge" style="background:var(--reading-bg);color:var(--reading-color);border-color:#fde68a">📖 ${{s.readingCount}}</span>`:''}}
        </div>
      </div>
      <div class="snapshot-actions">
        <button class="btn btn-secondary btn-sm" onclick="restoreSnapshot(${{s.id}})">↩ 복원</button>
        <button class="btn btn-sm" onclick="deleteSnapshot(${{s.id}})" style="color:var(--danger);border:1px solid var(--border);border-radius:var(--radius-xs);padding:5px 10px;font-size:12px;font-weight:600;background:var(--bg);transition:all var(--tr)" onmouseover="this.style.background='var(--danger)';this.style.color='#fff'" onmouseout="this.style.background='var(--bg)';this.style.color='var(--danger)'">🗑</button>
      </div>
    </div>`).join('')+`<div style="font-size:12px;color:var(--text-muted);margin-top:10px;text-align:right">${{snaps.length}}/${{MAX_SNAPSHOTS}}개 사용 중${{remain>0?` — ${{remain}}개 추가 가능`:'  (가득 참 — 가장 오래된 항목이 자동 삭제됩니다)'}}</div>`;
}}

/* ===================== GitHub Sync ===================== */
const GITHUB_KEY = 'bookshelf_github_v1';

function loadGithubSettings() {{
  try {{ return JSON.parse(localStorage.getItem(GITHUB_KEY)||'{{}}'); }} catch(e) {{ return {{}}; }}
}}
...
function saveGithubSettings() {{
  const s={{
    ...
    path:  document.getElementById('github-path')?.value.trim()||'bookshelf_backup.json',
  }};
  if(!s.token||!s.owner||!s.repo){{ showToast('⚠️ 토큰, 사용자명, 저장소명을 모두 입력해주세요.'); return; }}
  localStorage.setItem(GITHUB_KEY, JSON.stringify(s));
  showToast('✅ GitHub 설정이 저장되었습니다.');
  setGithubStatus('connected','🐙 GitHub 연동 완료: '+s.owner+'/'+s.repo);
}}

function setGithubStatus(type, msg) {{
  const bar=document.getElementById('github-status-bar');
  const txt=document.getElementById('github-status-text');
  if(!bar||!txt) return;
  bar.style.display='flex';
  bar.className='github-status-bar '+type;
  txt.textContent=msg;
}}
...
function populateGithubForm() {{
  ...
  set('github-token', s.token);
  set('github-owner', s.owner);
  set('github-repo',  s.repo);
  set('github-path',  s.path||'bookshelf_backup.json');
  if(s.token&&s.owner&&s.repo) setGithubStatus('connected','🐙 연동됨: '+s.owner+'/'+s.repo);
}}

function makeBackupJson() {{
  return JSON.stringify({{version:'1.0',exportDate:new Date().toISOString(),totalBooks:state.books.length,books:state.books}},null,2);
}}
...
function showGithubError(msg) {{
  const el=document.getElementById('github-detail-error');
  if(el){{ el.style.display='block'; el.textContent=msg; }}
  setGithubStatus('error','❌ '+msg.split('\\n')[0]);
}}
...
async function githubApiErrorMsg(resp) {{
  try {{
    const d=await resp.json();
    const parts=[`HTTP ${{resp.status}} ${{resp.statusText}}`];
    if(d.message) parts.push(`메시지: ${{d.message}}`);
    if(d.errors) parts.push(`상세: ${{JSON.stringify(d.errors)}}`);
    if(resp.status===401) parts.push('→ 토큰이 잘못되었거나 만료되었습니다. 새 토큰을 발급하세요.');
    if(resp.status===403) parts.push('→ 권한 부족. 토큰에 repo 또는 Contents:write 권한을 확인하세요.');
    if(resp.status===404) parts.push('→ 저장소를 찾을 수 없습니다. 사용자명/저장소명을 확인하세요.');
    if(resp.status===422) parts.push('→ 요청 형식 오류. SHA 값이 맞지 않을 수 있습니다.');
    return parts.join('\\n');
  }} catch(e) {{ return `HTTP ${{resp.status}} ${{resp.statusText}}`; }}
}}

async function testGithubConnection() {{
  const s=loadGithubSettings();
  if(!s.token||!s.owner||!s.repo){{ showToast('⚠️ 토큰, 사용자명, 저장소명을 먼저 입력하고 저장하세요.'); return; }}
  hideGithubError();
  setGithubStatus('syncing','🔍 연결 테스트 중...');

  // 1) 토큰 유효성
  try {{
    const r=await fetch('https://api.github.com/user',{{headers:githubHeaders(s.token)}});
    if(!r.ok) {{
      showGithubError(await githubApiErrorMsg(r));
      showToast('❌ 토큰 인증 실패'); return;
    }}
    const u=await r.json();
    if(u.login?.toLowerCase()!==s.owner.toLowerCase()) {{
      showGithubError(`토큰 사용자(${{u.login}})와 입력한 사용자명(${{s.owner}})이 다릅니다.`);
      showToast('❌ 사용자명 불일치'); return;
    }}
  }} catch(e) {{ showGithubError('네트워크 오류: '+e.message); showToast('❌ 네트워크 오류'); return; }}

  // 2) 저장소 접근
  try {{
    const r=await fetch(`https://api.github.com/repos/${{s.owner}}/${{s.repo}}`,{{headers:githubHeaders(s.token)}});
    if(!r.ok) {{
      showGithubError(await githubApiErrorMsg(r));
      showToast('❌ 저장소 접근 실패'); return;
    }}
    const repo=await r.json();
    hideGithubError();
    setGithubStatus('connected',`✅ 연결 성공: ${{repo.full_name}} (${{repo.private?'비공개':'공개'}})`);
    showToast(`✅ 연결 성공! ${{repo.full_name}}`);
  }} catch(e) {{ showGithubError('네트워크 오류: '+e.message); showToast('❌ 네트워크 오류'); }}
}}

async function syncToGitHub() {{
  const s=loadGithubSettings();
  if(!s.token||!s.owner||!s.repo){{ showToast('⚠️ 설정에서 GitHub 토큰을 먼저 입력해주세요.'); switchView('settings'); return; }}
  const path=s.path||'bookshelf_backup.json';
  const apiUrl=`https://api.github.com/repos/${{s.owner}}/${{s.repo}}/contents/${{path}}`;
  const headers=githubHeaders(s.token);

  hideGithubError();
  setGithubStatus('syncing','⏳ GitHub에 업로드 중...');
  ...
  try {{
    ...
    if(r.ok) {{
      ...
      // 404: 파일 없음(첫 업로드), 409: 저장소가 비어있음(커밋 없음) → sha 없이 진행
      sha='';
    }} else {{
      const msg=await githubApiErrorMsg(r);
      showGithubError('SHA 조회 실패\\n'+msg); showToast('❌ 업로드 실패'); return;
    }}
  }} catch(e) {{ showGithubError('네트워크 오류: '+e.message); showToast('❌ 네트워크 오류'); return; }}

  const body=JSON.stringify({{
    message:`📚 독서 데이터 백업 ${{new Date().toLocaleDateString('ko-KR')}} (${{state.books.length}}권)`,
    content:jsonToBase64(makeBackupJson()),
    ...(sha?{{sha}}:{{}})
  }});
  ...
  try {{
    ...
    if(r.ok) {{
      ...
      localStorage.setItem('bookshelf_github_synced',syncTime);
      hideGithubError();
      setGithubStatus('connected',`✅ 업로드 완료: ${{syncTime}}`);
      showToast('🐙 GitHub 업로드 완료!');
    }} else {{
      const msg=await githubApiErrorMsg(r);
      showGithubError(msg); showToast('❌ GitHub 업로드 실패');
    }}
  }} catch(e) {{
    showGithubError('네트워크 오류: '+e.message); showToast('❌ 네트워크 오류');
  }}
}}

function fillGithubPagesUrl() {{
  const s=loadGithubSettings();
  if(!s.owner||!s.repo) {{ showToast('⚠️ 설정에서 사용자명과 저장소명을 먼저 입력·저장하세요.'); return; }}
  const path=s.path||'bookshelf_backup.json';
  const url=`https://${{s.owner}}.github.io/${{s.repo}}/${{path}}`;
  const el=document.getElementById('restore-url-input');
  if(el) {{ el.value=url; showToast('📋 GitHub Pages URL이 입력되었습니다.'); }}
}}

async function restoreFromUrl() {{
  const url=(document.getElementById('restore-url-input')?.value||'').trim();
  if(!url) {{ showToast('⚠️ 복원할 URL을 입력해주세요.'); return; }}
  showToast('⏳ URL에서 데이터를 가져오는 중...');
  try {{
    const r=await fetch(url);
    if(!r.ok) {{
      const msg=r.status===404?'파일을 찾을 수 없습니다 (404). URL을 확인하세요.':r.status===403?'접근이 거부되었습니다 (403). 저장소가 공개인지 확인하세요.':`HTTP ${{r.status}} 오류`;
      showToast('❌ '+msg); return;
    }}
    const data=await r.json();
    const books=Array.isArray(data)?data:(data.books&&Array.isArray(data.books)?data.books:null);
    if(!books||!books.length) {{ showToast('❌ 올바른 백업 형식이 아닙니다.'); return; }}
    if(!confirm(`${{books.length}}권의 데이터를 복원하시겠습니까?\\n현재 데이터(${{state.books.length}}권)가 교체됩니다.`)) return;
    state.books=books;
    saveBooks();
    showToast(`✅ ${{books.length}}권이 복원되었습니다.`);
    setTimeout(()=>location.reload(),1500);
  }} catch(e) {{
    showToast('❌ 복원 실패: CORS 오류이거나 URL이 잘못되었습니다.');
  }}
}}
...
async function restoreFromGitHub() {{
  ...
  hideGithubError();
  showToast('⏳ GitHub에서 데이터를 가져오는 중...');
  try {{
    const r=await fetch(apiUrl,{{headers:githubHeaders(s.token)}});
    if(!r.ok) {{
      const msg=await githubApiErrorMsg(r);
      showGithubError(msg); showToast('❌ 파일을 가져올 수 없습니다.'); return;
    }}
    const d=await r.json();
    const json=base64ToJson(d.content);
    const data=JSON.parse(json);
    const books=Array.isArray(data)?data:(data.books&&Array.isArray(data.books)?data.books:null);
    if(!books?.length){{ showToast('❌ 올바른 백업 형식이 아닙니다.'); return; }}
    if(!confirm(`GitHub에서 ${{books.length}}권의 데이터를 복원하시겠습니까?\\n현재 데이터(${{state.books.length}}권)가 교체됩니다.`)) return;
    state.books=books; saveBooks();
    showToast(`✅ ${{books.length}}권이 GitHub에서 복원되었습니다.`);
    setTimeout(()=>location.reload(),1500);
  }} catch(e) {{
    showGithubError('복원 실패: '+e.message); showToast('❌ 복원 실패');
  }}
}}

/* ===================== Backup ===================== */
/* ---------- IndexedDB: 폴더 핸들 영구 저장 ---------- */
const IDB_NAME='bookshelf_idb', IDB_STORE='handles', IDB_KEY='backupFolder';
...
function updateFolderDisplay(name) {{
  ...
  if(el) el.textContent=name
    ? `📁 ${{name}} — 이 폴더에 백업이 저장됩니다`
    : '선택된 폴더 없음 — 브라우저 기본 다운로드 폴더에 저장됩니다';
}}
...
async function selectBackupFolder() {{
  ...
  try {{
    ...
    updateFolderDisplay(handle.name);
    showToast('📁 폴더가 저장되었습니다: '+handle.name);
  }} catch(e) {{
    if(e.name!=='AbortError') showToast('폴더 선택 실패: '+e.message);
  }}
}}

async function removeFolderSelection() {{
  state.directoryHandle=null;
  await clearHandleFromIDB();
  updateFolderDisplay('');
  showToast('폴더 지정이 해제되었습니다.');
}}
...
async function backupData() {{
  ...
  if(state.directoryHandle) {{
    const ok=await verifyFolderPermission(state.directoryHandle);
    if(ok) {{
      try {{
        ...
        return;
      }} catch(e) {{ showToast('⚠️ 폴더 저장 실패, 다운로드로 전환합니다.'); }}
    }} else {{
      showToast('⚠️ 폴더 접근 권한이 없어 다운로드로 저장합니다.');
    }}
  }}
  // 일반 다운로드
